...
#' @param samples a character vector of unique sample names to group the cells by. This is relevant only if multiple tumours are represented in the cna matrix, as it allows tumour-specific average CNA profiles to be computed in the call to infercna::cnaCor. See Details section for more information. Default: unique_sample_names(colnames(cna))
...
findMalignant = function(cna,
                         refCells = NULL,
                         samples = unique_sample_names(colnames(cna), max.nchar = 6),
                         gene.quantile = 0.9,
    ...
    if (verbose) {
        timerun = hms_span(start = old, end = new)
